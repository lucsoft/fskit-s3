...
/// Read a connection's secret from anywhere the app can — the shared access group
/// first, then the default keychain (where the store lands on an unsigned build). The
/// caller hands whatever it finds to the mount via `-o secret`; the shared-vs-default
/// distinction isn't useful, because the app being able to read a shared-group item
/// doesn't mean the sandboxed *extension* can.
pub fn read_secret(name: &str) -> Option<String> {
    let read = |shared| generic_password(options(name, shared)).ok();
    let bytes = read(true).or_else(|| read(false))?;
