    ...
    impl HealthController {
        ...
        fn apply_report(&self, _sender: Option<&AnyObject>) {
            ...
            if let Some(report) = taken {
                let iv = self.ivars();
                appkit::set_string(&iv.status, &health_line(&report.health));
                appkit::set_string(&iv.freshness, &freshness_line(&report.freshness));
...
/// A human-readable status line for the health state.
pub fn health_line(health: &Health) -> String {
    match health {
        Health::Ready => "🟢  Extension enabled and ready.".to_string(),
        Health::Disabled => {
            "🟠  Extension installed but not enabled. Open System Settings to enable it."
                .to_string()
        }
        Health::NotInstalled => {
            "🟠  Extension not registered yet — give it a moment, or relaunch the app.".to_string()
        }
        Health::Error(msg) => format!("🔴  Couldn't read extension status: {msg}"),
    }
}

/// A human-readable line for the build-freshness state (empty when not applicable).
fn freshness_line(freshness: &Freshness) -> String {
    match freshness {
        ...
        Freshness::Match { sha, dirty: true } => format!(
            "🟡  Registered build matches this app ({sha}) — dirty build, so equal SHAs aren't a guarantee."
        ),
        Freshness::Mismatch { registered, host } => format!(
            "⚠️  Build mismatch — fskitd will launch a DIFFERENT build.\n\
...
/// The short status shown on the menu's health row + the menu-bar glyph, derived
/// from a report. Returns `(glyph, row_text)`.
pub fn menu_line(report: &Report) -> (&'static str, String) {
    // A build mismatch is worth surfacing even when the extension is "ready".
    if matches!(report.freshness, Freshness::Mismatch { .. }) {
        return (
            "☁⚠️",
            "🟠  Extension ready — but a different build will run".to_string(),
        );
    }
    match &report.health {
        Health::Ready => ("☁", "🟢  Extension ready".to_string()),
        Health::Disabled => (
            "☁⚠️",
            "🟠  Extension not enabled — click to fix".to_string(),
        ),
        Health::NotInstalled => (
            "☁⚠️",
            "🟠  Extension not registered — click to fix".to_string(),
        ),
        Health::Error(_) => ("☁⚠️", "🔴  Extension status unavailable".to_string()),
    }
}
